...
export default function UserBanner() {
  ...
  const { requestuser } = router.query;

  useEffect(() => {
    if (reservation != null) showReservationlist();
    if (reservationuser != null) showReservationUserlist();
    if (requestuser != null) showRequestlist();
  });

  useEffect(() => {
    if (getCookie("first_name") == null) setUserName(null);
    else setUserName(getCookie("first_name"));
  }, [getCookie("first_name")]);

  const [role, setRole] = useState(null);
  useEffect(() => {
    if (getCookie("role") == null) setRole(null);
    else setRole(getCookie("role"));
  }, [getCookie("role")]);
  ...
  const uploadToClient = async (event) => {
    if (event.target.files && event.target.files[0]) {
      ...
      console.log(
        "🚀 ~ file: UserBanner.jsx:144 ~ uploadToClient ~ data:",
        data
      );
      ...
      <div className=" relative lg:w-2/3 ">
        <div className="bg-main bg-field1 flex items-start justify-end p-2 font-roboto bg-cover rounded-tl-12 h-[180px] shadow-md text-[10px] text-white">
          {userName != null && role == "host" && (
            <div
              onClick={showComplex}
              className="shcmp flex items-center space-x-1 bg-main py-1 px-2 rounded-full cursor-pointer"
            >
          ...
          {userName != null && role == "client" && (
            <div
              ...
              className="shcmp flex items-center space-x-1 bg-red-500 py-1 px-2 rounded-full cursor-pointer"
            >
              <i class="bx bxs-inbox text-xs"></i>
              <a>Show Request</a>
            </div>
          )}
        </div>
        <div className="flex justify-between absolute -bottom-20 w-full">
          <div className="flex mx-5 md:mx-10 space-x-4 items-end">
            <div className="relative bg-main group rounded-full w-[150px] h-[150px] flex items-center justify-center">
              <img
                src={createObjectURL}
                className=" rounded-full w-[150px] h-[150px] object-cover shadow-md"
              />
              <input
                type="file"
                name="myImage"
                onChange={uploadToClient}
                className="opacity-0 cursor-pointer absolute w-full h-full top-0 left-0 z-100"
              />
            ...
            <div className="flex flex-col pb-10 md:pb-6">
              <span>{userName}</span>
              <span className="text-xs hidden md:flex text-gray-500">
          ...
          <div className="flex flex-col md:flex-row items-center space-y-2 md:space-y-0 md:space-x-5 text-xs mt-20 md:mt-10">
            {userName != null && role == "host" && (
              <>
                <a onClick={showReservationlist} className="px-2 py-1 reslist">
                  Reservation List
                </a>
                <a onClick={showComplexlist} className="px-2 py-1 cmlist">
                  Complexe List
                </a>
              </>
            )}
            {userName != null && role == "client" && (
              <a
                onClick={showReservationUserlist}
                className="px-2 py-1 resuserlist"
              >
                My Reservations
              </a>
            )}
            <a
              ...
              className="activeOwner px-2 py-1 settings"
            >
              My Settings
            </a>
          </div>
